...
> - Примечание: Используйте GitBash(cmd не принимает значение порта) а иначе копируйте без порта:
> - go run ./cmd/main.go        
...
- Поддержка чисел с плавающей точкой.(Ввод с точкой{1.32}, а не запятой{1,32})
...
- 422 Unprocessable Entity: Ошибка в выражении. Возможные причины:
  - Неверный формат математического выражения.(Не целые числа вводяться через точку)
